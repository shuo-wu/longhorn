...
### Non-goals:
1. This feature is more responsible for fixing issue mentioned in [issue #1948](https://github.com/longhorn/longhorn/issues/1948#issuecomment-791641308).
2. This LEP won't take care of [backing image upload](https://github.com/longhorn/longhorn/issues/2404) and [backing image creation from Longhorn volume](https://github.com/longhorn/longhorn/issues/2403). 
...
2. Add a new CRD `backingimages.longhorn.io`.
    ...
    - Field `Status.DiskFileStateMap` reflect the actual file state for the disks.
    - Field `Status.DiskFileHandlingProgressMap` will report the file syncing progress.
...
5. Add a new controller `BackingImageDataSourceController`.
    1. Important notices:
        ...
        3. The state is designed to reflect the file state rather than the pod phase. Of course, the filed state will be considered as failed if the pod somehow doesn't work correctly.
    2. Workflow:
        ...
        5. Handle pod:
             1. Set state to `failed` if the pod does not work correctly.
             2. Start or stop monitoring based on pod phase.   
             3. Delete the errored pod.
             4. Recreate the pod. [TODO] For failed pod, there should be a retry limit and backoff interval. 
...
- The server will download the file immediately once the type is `download` and the server is up.
    ...
    - Each time when the image downloads a chunk of data, the progress will be updated. For the first time updating the progress, it means the downloading starts and the state will be updated to `in-progress`.    
##### manager service:
...
1. Launch a new page to present and operate BackingImages.
    ...
    2. Support batch deletion: Allow selecting multiple BackingImages; Add button `Deletion` on the top left.
    3. The columns on BackingImage list page should be: Name, Size, Created From (field `sourceType`), Details, Operation.
    4. Show more info for each BackingImage after clicking button `Detail`:
        - Present `Created From` (field `sourceType`) and the corresponding parameters `Parameters During Creation` (field `parameters`).
        - Show fields `expectedChecksum` and `currentChecksum` as `Expected SHA512 Checksum` and `Current SHA512 Checksum`.
        - Use a table to present the file info for each disk based on fields `diskFileStateMap` and `diskFileHandlingProgressMap`. The progress will be shown only when the state is `in-progress`.
    5. Add the following operations under button `Operation`:
        - `Delete`: No field is required. It should be disabled when there is one replica using the BackingImage.
        - `CleanupDiskImages`: This allows users to manually clean up the images in some disks in advance. The input of this API `disks` requires a list of disk UUID. I am thinking if we can put this operation on the `Detail` page.
    6. When a BackingImage is being deleted (field `deletionTimestamp` is not empty), show an icon behind the name which indicates the deletion state.
    7. If the state of all disk records are `failed`, use an icon behind the name to indicates the BackingImage unavailable.
